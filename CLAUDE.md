...
- `backend/transitions.py` — `resolve_transition(prev, current)` decides which of seven transition types applies, purely from slide data (never authored): `text_diff`, `scroll_only`, `file_switch`, `file_tree_change`, `fade`, `lerp_rect`, `rect_fade`. File Tree Change resolves but has no corresponding render function - real editors don't animate the file list, so an instant cut needs no animation code at all (this used to be File Switch's behavior, until File Switch gained its own highlight-fade render function - see below). The user's own authoring discipline - never two changes between consecutive slides at once, never a slide-to-slide with literally no change - is what makes "first matching condition wins" sufficient here, with no precedence/combination logic needed.
- `backend/transition_render.py` — one frame-sequence generator per transition type that *does* animate (`render_scroll_transition`, `render_text_diff_transition`, `render_file_switch_transition`, `render_fade_transition`, `render_lerp_rect_transition`, `render_rect_fade_transition`). 60fps (not the spec's stated 30 - deliberate). Motion (scroll, lerp_rect) eases via `ease_in_out_cubic`; fades stay linear (a plain cross-dissolve) - this includes `render_file_switch_transition` (cross-fades the sidebar's highlighted row between two otherwise-identical frames of the *new* file's content - the code pane itself cuts instantly, only the highlight animates) and `render_rect_fade_transition` (same image, rect's position fixed, only its visibility fades - for a rect appearing/disappearing, as opposed to `render_lerp_rect_transition` which is for an already-visible rect moving/resizing); Text Diff is golden-path only (no viewport lerp - assumes the edited region stays in view).
